...
layui.define(['element', 'common'], function(exports) {
	...
	Tab.prototype.exists = function(title) {
		...
		ELEM.titleBox.find('li').each(function(i, e) {
			...
			if($cite.text() === title) {
				tabIndex = i;
				layuipaid = $(this).attr("lay-id");
			};
		});
		return tabIndex+","+layuipaid;
	};
	/**
	 * 添加选择卡，如果选择卡存在则获取焦点
	 * @param {Object} data
	...
	Tab.prototype.tabAdd = function(data) {
		...
		var pslitab = tabIndex.split(",");
		tabIndex = pslitab[0];
		var existnum = pslitab[1];
		if(tabIndex === "-1") {
			globalTabIdIndex++;
			layuid++;
			...
			if(_config.closed) {
				...
				ELEM.titleBox.find('li').children('i.layui-tab-close[data-id=' + globalTabIdIndex + ']').on('click', function() {
					element.tabDelete(ELEM.tabFilter, $(this)[0].dataset.id).init();
				});
			};
			//切换到当前打开的选项卡
			element.tabChange(ELEM.tabFilter, layuid);
		} else {
			element.tabChange(ELEM.tabFilter, existnum);
			//自动刷新
