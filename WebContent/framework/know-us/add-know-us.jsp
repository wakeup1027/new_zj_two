...
<div class="layui-form-item">
    ...
    <div class="layui-input-inline">
      <select id="modeltype" class="select2" style="width:180px">
		<option value="1">湛江开发区情况</option>
		<option value="2">投资优势</option>
		<option value="3">经济发展状况</option>
		<option value="4">开发区规划</option>
	  </select>
    </div>
</div>
<!-- div class="layui-form-item">
...
function submitsave(){
	...
	if($("#savtype").val()=="edit"){
		$.ajax({
			url:"/framework/admin/updLiaoJieWoMen.action",
			type:"POST",
			data:{
				"title":title,
				"content":conne,
		...
		$.ajax({
			url:"/framework/admin/addLiaoJieWoMen.action",
			type:"POST",
			data:{
				"title":title,
				"content":conne,
...
if(ss!="0"){
	$.ajax({
		url:"/framework/admin/getLiaoJieWoMen.action",
		type:"POST",
		data:{
			"daid":ss
		},
