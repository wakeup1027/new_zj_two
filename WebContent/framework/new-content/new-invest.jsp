...
/*layui.use('upload', function(){
  ...
  var uploadInst = upload.render({
    ...
    url: 'framework/admin/addImages.action',
    done: function(res){
      	if(res.errno==0){
...
function submitsave(){
	...
	if($("#savtype").val()=="edit"){
		$.ajax({
			url:"/framework/admin/updTouZiDongTai.action",
			type:"POST",
			data:{
				"title":title,
				"content":conne,
		...
		$.ajax({
			url:"/framework/admin/addTouZiDongTai.action",
			type:"POST",
			data:{
				"title":title,
				"content":conne
...
if(ss!="0"){
	$.ajax({
		url:"/framework/admin/getTouZiDongTai.action",
		type:"POST",
		data:{
			"daid":ss
		},
