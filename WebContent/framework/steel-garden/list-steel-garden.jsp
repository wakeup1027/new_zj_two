...
layui.use(['table'], function(){
	...
	table.render({
	    ...
	    limit: 20,
	    url: "/framework/admin/listGangTieYuanQu.action", //数据接口
	    method:'post',
	    page: true, //开启分页
	    cols: [[ //表头
	...
	table.on('tool(test)', function(obj){
	    ...
	    if(obj.event === 'del'){
	      layer.confirm('真的删除行么', function(index){
	        $.ajax({
				url:"/framework/admin/delGangTieYuanQu.action?id="+data.id,
				type:"POST",
				data:{},
				dataType:"json",
	    	...
	    	window.location= "/framework/steel-garden/add-steel-garden.jsp?id="+data.id;
	      /*layer.prompt({
	        formType: 2
	        ,value: data.email
...
function new_date(){
	window.location= "/framework/steel-garden/add-steel-garden.jsp?id=0";
}
